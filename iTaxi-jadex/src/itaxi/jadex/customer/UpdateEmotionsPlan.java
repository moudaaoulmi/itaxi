...
public class UpdateEmotionsPlan extends Plan{
	...
	public static boolean isUpdate(CustomerState state, Date date){
		...
		System.out.println("date NOT null! State = " + state + " time = " + ((new Date()).getTime() - date.getTime()));
		
		
		if(state == CustomerState.HAPPY){// && (new Date()).getTime() - date.getTime() > 50){
			System.out.println("date NOT null! && HAPPY && >50 ->CHANGE STATE!");
			return true;
		}
		
		if(state == CustomerState.IMPACIENT){// && (new Date()).getTime() - date.getTime() > 100){
