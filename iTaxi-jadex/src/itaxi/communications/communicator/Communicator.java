...
public class Communicator extends Thread {
	...
	public void run() {
		...
		if (handler != null)
			System.out.println(handler.toString() + " waiting for connections on port:" + serverSocket.getLocalPort());
		else if (agent != null)
			System.out.println(agent.getComponentIdentifier().getName()
					+ " waiting for connections on port:" + serverSocket.getLocalPort());
				...
				if (agent != null) {
					agent.scheduleStep(new IComponentStep() {
						public Object execute(IInternalAccess ia) {
							...
							IInternalEvent event = scope.getEventbase().createInternalEvent(
									"taxi_nearby");

							event.getParameter("taxi").setValue((Object) msg);
